package com.ecom.order.config;

import com.ecom.order.client.InventoryClient;
import lombok.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.web.client.RestClient;
import org.springframework.web.client.support.RestClientAdapter;
import org.springframework.web.service.invoker.HttpServiceProxyFactory;

@Configuration
public class RestClientConfig {

    @Value("${inventory.url}")
    private String inventoryServiceUrl;

    @Bean
    public InventoryClient inventoryClient(){
        RestClient restClient = RestClient.builder().baseUrl(inventoryServiceUrl)
                .build();

        var resClientAdapter = RestClientAdapter.create(restClient);
        var httpSesssionProxyFactory = HttpServiceProxyFactory.builderFor(resClientAdapter).build();
        return httpSesssionProxyFactory.createClient(InventoryClient.class);
    }
}
